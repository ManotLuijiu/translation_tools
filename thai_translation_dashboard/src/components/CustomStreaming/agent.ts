...
import type { FileStatusData, ProgressData, StatusData } from "./types";
...
const analyzeDataTool = tool(
  async (
    ...
    const { dataSource, analysisType } = input as { dataSource: any; analysisType: any };
    const steps = [
...
const processFileTool = tool(
  async (input: unknown, config: ToolRuntime) => {
    const { filename, operation } = input as { filename: any; operation: any };
    /**
...
export const agent = createAgent({
  ...
  systemPrompt:
    "You are a helpful assistant that can analyze data and process files.",
});
