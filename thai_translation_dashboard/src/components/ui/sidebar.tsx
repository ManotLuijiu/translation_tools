    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          className={cn(
            'group/sidebar-wrapper relative flex min-h-svh w-full overflow-x-hidden has-[[data-variant=inset]]:bg-sidebar',
            className
          )}
          {...props}
        >
...
function Sidebar({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'duration-200 absolute inset-y-0 z-10 hidden h-svh transition-[left,right,width] ease-linear md:flex',
          // Width handling - use direct Tailwind classes instead of CSS custom properties
          state === 'collapsed'
            ? variant === 'floating' || variant === 'inset'
              ? 'w-[calc(3rem_+_theme(spacing.4)_+2px)] p-2'
              : 'w-12' // 3rem = 48px
            : 'w-64', // 16rem = 256px
          side === 'left'
          ...
          // Border handling
          state !== 'collapsed' && (side === 'left' ? 'border-r' : 'border-l'),
          className
        )}
        {...props}
      >
...
// App-specific sidebar component for Translation Tools
// Note: This component is defined here following M Capital SPA pattern
// DO NOT import this from external files - it must be local to this SPA
...
>((props, ref) => {
  ...
  return (
    <Sidebar id="sidebar__app__wrapper" collapsible="icon" {...props} ref={ref}>
      ...
      <SidebarContent>
        <SidebarGroup>
          <SidebarGroupLabel>{__('Navigation')}</SidebarGroupLabel>
          <SidebarMenu>
            {navItems.map((item) => (
