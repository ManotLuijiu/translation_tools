...
class WorkspaceManager:
    ...
    def get_default_integrations_content():
        ...
        return json.dumps([
            {"id": "header", "type": "header", "data": {"text": "<span class=\"h4\"><b>Reports & Masters</b></span>", "col": 12}},
            {"id": "backup", "type": "card", "data": {"card_name": "Backup", "col": 4}},
            {"id": "google", "type": "card", "data": {"card_name": "Google Services", "col": 4}},
            {"id": "auth", "type": "card", "data": {"card_name": "Authentication", "col": 4}},
            {"id": "settings", "type": "card", "data": {"card_name": "Settings", "col": 4}},
            {"id": "push", "type": "card", "data": {"card_name": "Push Notifications", "col": 4}},
        ])
        ...
        try:
            ...
            for link_data in app_links:
                ...
                for existing_link in workspace.links:
                    if existing_link.label == link_data.get("label") and existing_link.link_to == link_data.get("link_to"):
                        link_exists = True
                        break
    ...
    def create_app_workspace(app_name, workspace_config):
        ...
        try:
            ...
            if not frappe.db.exists("Workspace", workspace_name):
                ...
                if content:
                    workspace.content = json.dumps(content) if isinstance(content, list) else content
            else:
    ...
    def setup_translation_tools_links():
        ...
        try:
            ...
            if success:
                logger.info("✅ Translation Tools links added to Integrations workspace")
                return {"success": True, "message": "Translation Tools links added"}
            else:
                return {"success": False, "message": "Failed to add Translation Tools links"}
    ...
    def setup_thai_business_suite_links():
        ...
        try:
            ...
            if success:
                logger.info("✅ Thai Business Suite links added to Integrations workspace")
                return {"success": True, "message": "Thai Business Suite links added"}
            else:
                return {"success": False, "message": "Failed to add Thai Business Suite links"}
