    ...
    for i, component in enumerate(components):
        try:
            ...
            if not check_module_exists(
                component["module_path"], component["function_name"]
            ):
                logger.error(
                    f"Skipping {component['description']} due to missing module"
                )
                continue
...
def import_initial_data():
    ...
    try:
        ...
        if check_module_exists(
            "translation_tools.translation_tools.setup.import_default_glossary",
            "import_default_glossary_terms",
        ):
            from translation_tools.setup.import_default_glossary import (
                import_default_glossary_terms,
            )
...
def validate_configuration():
    ...
    if not os.path.exists(config_file):
        config_issues.append(
            _(
                "API key configuration file not found. Translation features may not work."
            )
        )
        ...
        with open(config_file, "r") as f:
            ...
            if (
                "OPENAI_API_KEY" not in config_content
                and "ANTHROPIC_API_KEY" not in config_content
            ):
    ...
    if missing_doctypes:
        config_issues.append(
            _("Missing required DocTypes: {0}").format(", ".join(missing_doctypes))
        )
    ...
    if config_issues:
        ...
        issues_html = (
            "<ul>" + "".join([f"<li>{issue}</li>" for issue in config_issues]) + "</ul>"
        )
        frappe.msgprint(
            _("Translation Tools was installed, but with some configuration issues:")
            + issues_html,
